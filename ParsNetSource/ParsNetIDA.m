...
function [ParsNetwork,Agmm,parameter,performance] = ParsNetIDA(data,nin,mode,dType,chunkSize)
...
for iFolds = 1:nFolds
    if iFolds == 1
        % only initial batch has label
        dataProportion = 1;
        initialData  = Data{iFolds}(:,1:nin);
        initialLabel = Data{iFolds}(:,nin+1:end);
    elseif iFolds > 1
        dataProportion = 0;
    end
    ...
    %% prepare data for the remaining batch
    if iFolds > 1
        inputLabeledData = initialData;
        targetLabeledData = initialLabel;
        inputData = inputUnlabeledData;
        targetData = targetUnlabeledData;
        parameter.x = inputData;
        parameter.T = targetData;
        parameter.inputLabeledData = initialData;
        parameter.targetLabeledData = initialLabel;
        noOfLabeledData = 0;
    end
    ...
    if iFolds > 1
        augmentedIndices = find(augmentedLabelIdentity==1);
        augmentedInputLabeledData  = augmentedInputLabeledData(augmentedIndices,:);
        augmentedTargetLabeledData = augmentedTargetLabeledData(augmentedIndices,:);
        nLabeledData = size(augmentedInputLabeledData,1);
        inputData = [inputUnlabeledData;augmentedInputLabeledData];
        targetData = [targetUnlabeledData;augmentedTargetLabeledData];
        augmentedLabelIdentity     = augmentedLabelIdentity(augmentedIndices);
    end
    ...
    [ParsNetwork,Agmm,parameter] = ParsNet_generativeIDA(ParsNetwork,...
