...
function [ParsNetwork,gmm,parameter] = ParsNet_generative(ParsNetwork,...
    gmm,parameter,x,x_ori,targetLabeledData,nLabeledData)
...
for iData = 1:nData
    ...
    if gmm.calculation == 1
        if iData > 1 && iData <= nx_ori
            gmm = AGMM(gmm,x_ori(iData,:),nin,meanNetBias2(iData-1),...
                evaluationWindow,nLabeledData,iData,mode);
            if iData <= nLabeledData % only updated when there are labels
